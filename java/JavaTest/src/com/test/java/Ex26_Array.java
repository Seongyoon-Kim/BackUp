...
//							+ n2[(int)(Math.random() * n1.length)]);
	...
	private static void m10() {
		...
		//지역 변수(***)는 초기화 하지 않으면 사용이 불가능하다.
		//System.out.println(n);
		//System.out.println(s);
		
		//초기화 해야할 값이 정해지지 않았지만 일단 초기화를 해야하는 경우
	...
	private static void m8() {
		...
		int[] num4 = new int[3]; //배열은 꼭 만들어야한다!!
	...
	private static void m5() throws NumberFormatException, IOException {
		...
		//배열의 성질
		// - 배열의 길이가 불변(Immutable)이다. > 한번 만들어진 배열의 방 개수는 수정이 불가능하다. > 방을 더 만들거나, 있는 방을 삭제할 수 없다. > 배열에 들어갈 데이터의 개수를 미리 충분히 예측해서 최대값의 근접한 길이로 잡는다.(조금 더 크게)
	...
	private static void m3() {
		...
		int[] num = new int[10];
		//int num2[] = new int[10]; -> 옛날 작성법. 이걸로 하지 말자.
