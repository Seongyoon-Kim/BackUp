...
public class Ex56_Cast {
	...
	public static void main(String[] args) {
		...
		//2. 브랜드 증가 > Dell500, BenQ600, Epson700.. 10여종 증가
	...
	private static void m3() {
		...
		//Printer <-> LG300, HP400
		Printer p1 = new LG300(); //부모 클래스 = 자식 클래스 > 업캐스팅
		Printer p2 = new HP400(); //부모 클래스 = 자식 클래스 > 업캐스팅
		...
		for (int i=0; i<list.length; i++) {
			...
			//추상메소드 -> 겉으로는 동일해보여도 실제로는 하는일이 각각 다르다.
			list[i].print();
			
			//공통기능 -> 구현 상속 메소드 -> 겉으로는 동일하고.. 실제로 하는일도 동일하다.
			//list[i].info();
...
//LG300, HP400 -> 프린터
//1. 공통된 정보(데이터), 기능(행동, 메소드) -> 상속 -> 클래스 or 추상클래스 or 인터페이스(X)
...
abstract class Printer {
	...
	//구현 멤버
	// - 회사나 모델에 상관 없이 모든 프린터가 가져야 할 공통 구현 기능
	public String model;
	public int price;
	public int ink;
	
	public void info() {
		System.out.printf("모델명: %s%n, 잉크량: %dml%n, 가격: %d원%n", this.model, this.price, this.ink);
	}
	
	
	//추상 멤버
	// - 회사나 모델에 상관 없이 모든 프린터가 가져야 할 공통적인 사용법
...
class LG300 extends Printer {
	...
	//모든 프린터의 사용법을 통일하기 위해서 만든 멤버
	@Override
	public void print() {
		//내용만은 내맘대로.. LG 맘대로.. LG 독자적인 기술 구현
