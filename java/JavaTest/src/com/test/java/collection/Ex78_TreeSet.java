...
public class Ex78_TreeSet {
	...
	public static void main(String[] args) {
		...
		List
		- ArrayList: Array(배열형태의 공간) + List 사용법 제공(index)
		...
		TreeSet
		- Tree 구조를 가지는 Set
		- 중복값 X, 순서(index) X, 정렬X
		- 이진 트리 구조 기반의 자동 정렬이 되어있는 Set
		- 정렬 필요 + 중복값 X + 검색 자주
		...
		*/
		...
		//메소드
		System.out.println(set.first()); //최소값
		System.out.println(set.last()); //최대값
		...
		//******************************************************************
		//배열 안에 객체가 들었다. <-> 배열 안에 원자값이 들어있다.(int, String..)
		...
		list.sort(new Comparator<Score>() {
			...
			public int compare(Score o1, Score o2) {
				...
				//return o1.getKor() - o2.getKor(); //국어 + 오른차순
		...
		tset.add(new Score("하하하", 30, 50, 40));
		tset.add(new Score("호호호", 90, 20, 30));
...
class Score implements Comparable<Score>{
	...
	public int compareTo(Score o) {
		...
		//return (this.getKor() + this.getEng() + this.getMath()) - (o.getKor() + o.getEng() + o.getMath());
