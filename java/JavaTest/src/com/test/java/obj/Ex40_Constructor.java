	...
	public static void main(String[] args) {
		...
		- 차이점
			a. 생성자: 객체가 처음 만들어질 때 자동으로 호출된다.
				- 두번 이상 호출이 불가능하다.(객체가 태어날 때 딱 1번만 실행되고 그 이후에 호출이 불가능하다.
				- *** 역할: 초기화
		...
		*/
		...
		Person p3 = new Person("홍길동", 20); //주문 생산이 가능해짐. 원하는 모습으로 생성
		...
		Person p4 = new Person("아무개", 25); //익명, -1
		...
		// -> 정적 멤버의 초기화는 객체 생성 유무와 상관 없이 별도로 실행되어야 한다.
...
객체 변수의 생명주기
- new, 객체 생성(생성) ~ 객체의 참조가 모두 끊겨서가비지가 되고, GC가 그 객체를 회수하는 순간(소멸)

정적 변수의 생명주기
- main() 메소드 실행 전(생성) ~ main() 메소드 종료 후(소멸)
