...
insert into tblstaff (seq, name, salary, address, project) values (2, '아무개', 150, '부산시', 'tv광고');
insert into tblstaff (seq, name, salary, address, project) values (3, '하하하', 250, '대전시', '매출분석');

-- 홍길동 + 프로젝트 1건 추가(고객관리)
insert into tblstaff (seq, name, salary, address, project) values (4, '홍길동', 300, '서울시', '고개관리');
...
- 조인에서 컬럼은 반드시 테이블명(소유주)을 적는게 좋다.(충돌 발생 방지)

조인의 종류(ANSI-SQL)
1. 단순 조인, Cross Join
    - 사용을 거의 안함 > 결과셋에 유효한 레코드와 유효하지 않은 레코드가 뒤섞여 있기 때문
...
-- Sub Query(상관 서브 쿼리)
select item, qty, cseq, (select name from tblCustomer where tblCustomer.seq = tblSales.cseq) as name, (select tel from tblCustomer where tblCustomer.seq = tblSales.cseq) as tel from tblSales;


insert into tblStaff(seq, name, salary, address) values (1, '홍길동', 300, '서울시');
insert into tblStaff(seq, name, salary, address) values (5, '후후후', 100, '서울시');
...
select
    s.name as 직원명, s.salary as 급여, s.address as 주소, p.project as 프로젝트
from tblStaff s
...
from tblVideo v
    left outer join tblRent r
        on v.seq = r.video
            group by v.name
                order by count(r.rentdate) desc, v.name;
