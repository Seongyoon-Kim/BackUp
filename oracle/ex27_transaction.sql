...
트랜잭션, Transaction
- 오라클에서 트랜잭션을 처리하는 방법
- 오라클에서 발생하는 1개 이상의 명령들을 하나의 논리 집합으로 묶어 놓는 단위 > (실행된) 명령어들의 집합 > 통제(제어)
- 트랜잭센여 포함되는 명령어 > insert, update, delete > 테이블 조작(변경)
...
commit; -- 현재 트랜잭션의 모든 작업을 실제 데이터베이스(오라클)에 적용하라.
