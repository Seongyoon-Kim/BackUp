...
- 실제 프로젝트 -> 일부 사용 O, 일부 사용 X
- 교육(취업) 프로젝트 -> 골고루 사용 O -> X -> 포트폴리오에 들어갈 정도의 분량
...
2. 저장 함수
    - 반드시 매개변수가 있어야 한다.(개수는 0개 이상, 일반적으로는 1개 이상)
...
-- 반환값을 받는 방식의 차이 때문에 사용 되는 용도가 다르다.
-- PL/SQL(X)
...
-- 저장 프로시저
-- out parameter
-- 1. 결과셋의 레코드가 1개일 때 대응
-- 2. 결과셋의 레코드가 N개일 때 대응
create or replace procedure procTest (
    pnum in number,         -- 직원번호
...
-- 부서 -> 목록 반환
-- 1. 저장 프로시저(커서 반환 가능, ANSI-SQL 자료형 + PL/SQL 자료형(커서))
...
-- 퇴사
--ORA-02292: integrity constraint (HR.SYS_C007259) violated - child record found
...
-- 그다지 좋은 방법은 아님.
-- 자식이 있는 부모 테이블을 삭제하는 방법(부모를 강제로 삭제,
...
begin
    ...
    if to_char(sysdate, 'dy') = '목' then
        ...
        -- -20000 ~ 29999
        raise_application_error(-20001, '목요일에는 퇴사가 불가능 합니다.');
...
update tblMen set weight = 70 where name = '하하하';
...
1. 생략
    - 문장 단위 트리거
    - 트리거 실행 횟수 1회
    - DML에 의해서 적용된 행의 개수와 무관하게 단 1회만 실행
    - 목적: 행동 자체가 중요한 트리거(적용된 레코드(데이터)는 그다지 중요하지 않다.)

2. 사용
    - 행 단위 트리거
    - 트리거 실행 횟수 N회
    - DML에 의해서 적용된 행의 개수와 동일한 횟수만큼 실행
...
create table tblComment (
    ...
    subject varchar2(100) not null,                 -- 댓글 제목
    regdate date default sysdate not null,          -- 날짜
...
-- 인덱스가 없는 상태에서 검색
select distinct name from tblIndex where name = '김희예'; -- 00:00:01.190
...
-- 
select distiㄹnct job from tblIndex where job = '게임기획자';
...
5. 해당 컬럼이 null을 포함하는 경우(색인 테이블은 null 제외)
...
-- 1. 비고유 인덱스
-- : 색인의 값이 중복이 가능하다.
create index idx_tblindx_buseo on tblInsa(buseo);
...
-- 3~4개 추가 + 상황 정리
-- 프로젝트 -> 맨 마지막에 작업 -> 만든 SQL에서 자주 보이는 where절을 찾아서 -> 몇개 정도 인덱스로 생성
--                            -> where절 유일 -> 업무상 호출 횟수 많다. -> 인덱스 생성
