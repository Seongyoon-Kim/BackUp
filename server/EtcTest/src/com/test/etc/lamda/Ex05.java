...
public class Ex05 {
	...
	public static void main(String[] args) {
		...
		// 1. EtcTest > lib > data-0.1.1.jar 복사
		// 2. EtcTest > 오른쪽 버튼 > Build Path > Configure Build Path
		// 3. Libraries < Classpath 선택 > Add jars 클릭 > data-0.1.1.jar 선택
		...
		// b. 최종 처리: 합계, 평균, 카운트, 최대값, 최소값, forEach 등
		//m4();
		//m5();
...
//		System.out.println(ulist.get(0));
	...
	private static void m5() {
		...
		int n = 1;
//		list.stream().distinct().forEach(num -> System.out.println(n + ":" + num));
//		System.out.println();
		...
		List<String> names = Arrays.asList("홍길동", "아무개", "김훈",  "하하하", "홍길동", "호호호", "김훈", "아무개", "테스트");
	...
	private static void m4() {
		...
		Data.getUserList().stream().filter(user -> user.getHeight() > 170 && user.getWeight() > 70).forEach(user -> System.out.println(user));
		System.out.println();
...
//		System.out.println(Data.getItemList());
		...
		//노란색 제품만
		Data.getItemList().stream().filter(item -> item.getColor() == Color.YELLOW).forEach(item -> System.out.printf("%s(%s, %d)\n", item.getName(), item.getColor(), item.getSize()));
		System.out.println();
	...
	private static void m3() {
		...
		try {
			...
			java.nio.file.Files.lines(Paths.get(".\\data\\data.txt")).forEach(ln -> System.out.println(ln));
			System.out.println();
...
//			File[] list = dir.listFiles();
			...
			Path workspace = Paths.get("D:\\class\\server"); // 현재 폴더
			java.nio.file.Files.list(workspace).forEach(path -> System.out.println(path.toString())); // Stream<Path>
			System.out.println();
...
//		System.out.println(list);
	...
	private static void m1() {
		 ...
		 * 
		 * list.forEach(function(index, item) { //배열의 요소를 하나 념거준 뒤 함수 한번 호출 x 5번 });
		 * 
		 * //람다식(X) -> 화살표 함수(Arrow Function) list.forEach(() => {
		 * 
		 * });
		 * 
		 */
