...
<body>
	...
	<section class="main-section">
		...
		<div class="well">
			<form action="/myapp/openapi/book.do" method="GET">
				<!-- <form> 내에 텍스트 박스가 1개만 존재하면 그 텍스트 박스에서 엔터를 치면 submit 기능이 동작된다. -->
				<input type="text" name="query" id="" class="form-control" placeholder="검색어를 입력하세요." />
			</form>
		</div>
			...
			<c:if test="${not empty total && total > 0}">
				<c:forEach items="${list}" var="book">
					<tr>
						...
						<td>
							<div style="text-decoration: line-through;"><fmt:formatNumber value="${book.price}" pattern="#,###" />원</div>
