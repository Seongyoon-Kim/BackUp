...
public class BoardDAO {
	...
	public ArrayList<CommentDTO> listComment(String seq) {
		...
		try {
			...
			//부모글번호를 조건으로..
			String sql = "select c.*, (select name from tblUsers where id = c.id) as name from tblComments c where pseq = ? order by seq desc";
	...
	// DelOk 서블릿이 글 번호를 줄테니 글 번호를 부모로 하는 모든 댓글을 삭제해주세요
	public void delAllComment(String seq) {
		
		try {
			
			String sql = "delete from tblComments where pseq = ?";
